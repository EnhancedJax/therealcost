...
export const priceRegex =
  /(?:((?:XY|XYZ)\s?[$£¥€]?|[$£¥€]))\s?(\d+(?:,\d{3})*(?:[.,]\d{0,2})?)((?:\s?[Mm]illion|\s?[Mm]il|[Mm]|\s?[Tt]housand|[Kk])?)/m;
export const priceRangeRegex =
  /(?:((?:XY|XYZ)\s?[$£¥€]?|[$£¥€]))\s?\d+(?:,\d{3})*(?:[.,]\d{0,2})?(?:\s?[Mm]illion|\s?[Mm]il|[Mm]|\s?[Tt]housand|[Kk])?\s?-\s?(\d+(?:,\d{3})*(?:[.,]\d{0,2})?)((?:\s?[Mm]illion|\s?[Mm]il|[Mm]|\s?[Tt]housand|[Kk])?)/m;
